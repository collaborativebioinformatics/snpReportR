gene.names<-c("SCNN1A", "SCNN1B", "SCNN1G", "SCNN1D")
...
for(i in 1:length(gene.names)){
  ...
  assign(paste(gene.names[i],"Associated.Diseases", sep="."), gene.disease[-which(duplicated(gene.disease$Gene.alleles.diseases.name)),which(colnames(gene.disease)=="Gene.alleles.diseases.name")])
  ...
  assign(paste(gene.names[i],"Top.Expression", sep="."), gene.expression[head(order(gene.expression$Gene.rnaSeqResults.expressionScore, decreasing = T), n=5),which(colnames(gene.expression)=="Gene.rnaSeqResults.tissue")])
  ...
  assign(paste(gene.names[i], "Recent.Publications", sep = "."), gene.publication[1:5,which(colnames(gene.publication)%in%c("Gene.publications.firstAuthor","Gene.publications.year","Gene.publications.pubMedId"))])
}

# Associated diseases
assign(gene.disease, get(paste(gene.names[1],"Disease", sep=".")))
