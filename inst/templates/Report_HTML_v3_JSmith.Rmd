...
top.gene.expn <- annots[grep(gene.expn.regex, names(annots))] %>%
  bind_rows() %>% 
  mutate(Expression_Rank=1:nrow(.)) %>% 
  dplyr::select(Expression_Rank, everything())
...
```{r  fig.width=10, fig.height=10, echo=FALSE, fig.cap="Expression of Genes with identified variants/SNVs in Normal Tissues."}
...
top.gene.expn.val <- annots[grep(gene.expn.val.regex, names(annots))] %>%
  bind_rows() %>% 
  mutate_at(vars(Gene.rnaSeqResults.expressionScore), ~as.numeric(.)) %>% 
...
bar.plot <- ggplot(top.gene.expn.val, aes(tidytext::reorder_within(Gene.rnaSeqResults.tissue,
  ...
  coord_flip() +
  theme_classic() +
  labs(x="Tissue", y="Expression Score", title="Expression in Normal Tissues (TPM)") +
...
bar.plot %>% 
  plotly::ggplotly(width=900, height=700)
```
...
```{r message=FALSE,fig.width=3, fig.height=3, echo=FALSE, fig.align='left', fig.show=TRUE, fig.cap="Figure 1. Percentage of different types of mutations identified."}
...
```{r fig.width=7,fig.height=5, fig.show=TRUE, echo=FALSE, fig.subcap="A vizualization of the SNVs", message=FALSE, warning=FALSE}
...
```{r echo=FALSE, fig.height=5, fig.width=5, fig.show=TRUE, fig.cap="Expression of the genes with pathogenic variants identified."}
